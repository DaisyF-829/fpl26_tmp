...
`--pl_baseline`：不加载模型。仅在末节点（fanout==0 且 y_valid，与原先「末节点」指标一致）上，
用原始 tnode_pl_time与 tnode_rt_time 对比，计算 R² 与 MAPE；不跑 Coverage/Top-K 等。
"""
...
def _pl_baseline_arrays_from_npz(npz_path: str, N: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, float]:
    """
    与 data_loader.load_timing_graph 相同的键访问方式（name in z.files 再读，缺键用默认），
    避免旧版 npz 无 tnode_pl_time 时 z['tnode_pl_time'] 抛错。

    返回 pred_pl_raw（nan=无效 PL）、rt_raw、pl_ok、pl_cpd_hat；时间单位与 npz 一致。
    """
    ...
    try:
        ...
        pl_time = arr("tnode_pl_time", np.full(N, -1.0, dtype=np.float32))
        pl_time = np.asarray(pl_time, dtype=np.float64).reshape(-1)[:N]
        pl_raw = arr("tnode_pl_arrival_mask", arr("tnode_pl_valid", None))
        if pl_raw is None:
            pv = np.zeros(N, dtype=np.float64)
        else:
            pv = np.asarray(pl_raw, dtype=np.float64).reshape(-1)[:N]
        pl_ok = (pv > 0) & np.isfinite(pl_time) & (pl_time >= 0.0)
        if pl_ok.any():
...
def evaluate_one(
    ...
    *,
    ...
    with torch.no_grad():
        if pl_baseline:
            pred_arrival_ns, rt_raw, pl_ok_arr, _ = _pl_baseline_arrays_from_npz(
                npz_path, N
            )
    ...
    if pl_baseline:
        ...
        # 与历史版本一致：末节点 = fanout==0 ∩ y_valid；勿用 tnode_type==1（不少数据里无 SINK 或 PL 不在 SINK）
        ...
        if not silent:
            print(
                "\n[PL 基线] 末节点（fanout==0且 y_valid）∩有效PL：tnode_pl_time vs tnode_rt_time（未归一化）",
                flush=True,
            )
            print(f"可评估终点数: {n_end}", flush=True)
            if m_all is not None:
                print(f"R²={m_all['r2']:.4f}  MAPE={m_all['mape']:.2f}%", flush=True)
            else:
                print("（可评估终点 < 2，不计算 R²/MAPE）", flush=True)
        if save_pred:
    ...
    if save_pred:
        ...
        cpd0 = float(data.cpd.reshape(-1)[0].item())
        plm0 = float(plm.reshape(-1)[0].item())
        pred_cpd_out = float(np.exp(pg0) * plm0)
        np.savez(
            out_path,
            pred_arrival=pred_arrival_ns.astype(np.float32),
...
def evaluate_directory(
    ...
    *,
    ...
    if pl_baseline:
        model = None
        print(
            "PL 基线：用 tnode_pl_time 作为节点预测，跳过模型加载。",
            flush=True,
        )
    else:
    ...
    if not pl_baseline:
        ...
        # 你要的：所有 npz 的平均 top20 overlap（单文件内均值再宏平均）
        ...
        print("PL 基线：不汇总 Coverage / Top20 / 图头（仅末节点 fanout==0 的 R² 与 MAPE）。", flush=True)

    m_all_label = "全图监督节点（每图 y_valid）"
    if pl_baseline:
        m_all_label = (
            "全图监督节点（每图 y_valid）[PL基线=末节点fanout0，tnode_pl_time vs rt]"
        )
    ...
    if pl_baseline:
        print(
            f"PL 基线末节点可评估点数（每图计数之和）: {total_leaf_points}",
            flush=True,
        )
        return
    if n_leaf_ok > 0:
...
def main() -> None:
    ...
    ap.add_argument(
        "--pl_baseline",
        action="store_true",
        help="不加载 --model；仅末节点 fanout==0：tnode_pl_time vs tnode_rt_time 的 R² 与 MAPE",
    )
    args = ap.parse_args()
