...
`--pl_baseline`：不加载模型。有效 PL 仅由 tnode_pl_time>0（且有限）判定，不读 PL 掩码键。
末节点（fanout==0∩y_valid）上 tnode_pl_time vs tnode_rt_time 的 R²/MAPE，并照常跑 Top-K/Top20 路径与 Coverage。
"""
...
def _pl_baseline_arrays_from_npz(npz_path: str, N: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, float]:
    """
    与 load_timing_graph 相同方式读 tnode_pl_time / tnode_rt_time。
    pl_baseline 专用：不依赖 tnode_pl_valid等掩码键，仅用 finite(pl_time) & (pl_time > 0) 为有效 PL。
    """
...
def evaluate_one(
    ...
    *,
    ...
    if pl_baseline:
        ...
        if not silent:
            print(
                "\n[PL 基线] 有效PL=tnode_pl_time>0；末节点 fanout==0：tnode_pl_time vs tnode_rt_time（未归一化）",
                flush=True,
            )
