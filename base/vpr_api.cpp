...
bool vpr_flow(t_vpr_setup& vpr_setup, t_arch& arch) {
    ...
    // if (vpr_setup.AnalysisOpts.write_heterographs){
    //     tgnn::HeteroGraph::get_instance().build_from_timing_graph_prepl();
    //     VTR_LOG("Successfully Build heterogeneous graph.\n");
    // }
    tgnn::HeteroGraph::get_instance().build_from_timing_graph_prepl();
    VTR_LOG("Successfully Build heterogeneous graph.\n");
    ...
    tgnn::HeteroGraph::get_instance().write_timing_graph_npz();
    VTR_LOG("Successfully write heterogeneous graph.\n");
...
void vpr_analysis(const Netlist<>& net_list,
    ...
    if (vpr_setup.TimingEnabled) {
        ...
        tgnn::HeteroGraph::get_instance().build_rr_node_map();
        tgnn::HeteroGraph::get_instance().extract_arrival_time_aftroute(*timing_info);
        tgnn::HeteroGraph::get_instance().extract_top_k_paths(*timing_info, /*K=*/20);
        tgnn::HeteroGraph::get_instance().extract_node_features();
        tgnn::HeteroGraph::get_instance().enrich_edge_features(net_delay);
